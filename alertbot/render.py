...
def section_summary(win, fl=None, cmp=None, events=None, kr_upjong=None,
    ...
    # 진입 타이밍 (1430/1900 전용). 1430 은 3지점 비교(14:30/15:20 종가/20시 NXT 종가,
    # 09-22 퍼프 30분봉 EWY 131일·삼전/하이닉스 79일 실측):
    #   강세(+0.3%↑): 마감까지 되돌림(-0.16~-0.36%p)으로 15:20 이 최저가. 저녁(15:30→20시)
    #     은 반등(+0.15~+0.52%p)이라 20시는 되레 비쌈 → 15:20 대기, 20시 넘기지 말 것.
    #   중립(±0.3%): 마감(+0.50%p, 상승 80%)·20시(+0.61%p)까지 계속 상승 → 지금이 최저가.
    #     (EWY n=10 소표본, 09-16 68일 분석과 방향 일치)
    #   약세(-0.3%↓): 지수는 15:20~20시 무차별(차이 0.05%p), 개별 주도주는 저녁 반등
    #     (+0.37~+0.40%p)이라 역시 15:20 → 15:20 대기.
    # 1900 은 기존 09-16 실측(중립 승률 81%, 강세 되돌림, 약세 막판 반등) 유지.
    timing = None
    slot = win.get("slot")
